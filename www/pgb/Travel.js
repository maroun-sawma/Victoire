...
var Member_family = function (_INSUREDTRAVEL_MEMBERS_ID, _MEMBER_NAME, _MEMBER_BIRTHDATE, _MEMBER_AGE, _MEMBER_GENDER, _MEMBER_PASSPORT, _MEMBER_PREMIUM,
    ...
    self._Travel_Errors = ko.validation.group(self); // USED FOR KO VALIDATION //
};
/*---------------------------------------------------------------*/

var List_CountyOfResidance = [{ "COUNTRIES_ID": 14, "COUNTRIES_NAME": "Lebanon" }];
MyApp_GVM.List_TravelCountyOfResidance = ko.mapping.fromJS(List_CountyOfResidance);
...
MyApp_GVM.List_Members = ko.mapping.fromJS(List_Members);
...
$(document).on("pagebeforecreate", "#pg_Travel", function (event, ui) {
    ...
    $('#Txt_Tr_Birthdate').mobiscroll().date({
        ...
        minDate: new Date(yyyy-84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    $('#Txt_Tr_EffectiveDate').mobiscroll().date({
        ...
        onSelect: function (valueText, inst) {
            ...
            ko.mapping.fromJS(List_TravelPLans, MyApp_GVM.List_TravelPLans);

        },
    });

    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_1').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_2').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_3').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_4').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_5').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd), // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_6').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    // Mobiscroll Date & Time initialization
    $('#txt_TravelDOB_7').mobiscroll().date({
        theme: 'android-holo-light',        // Specify theme like: theme: 'ios' or omit setting to use default 
        display: 'bubble',    // Specify display mode like: display: 'bottom' or omit setting to use default
        mode: 'mixed',          // More info about mode: http://docs.mobiscroll.com/2-17-1/datetime#!opt-mode
        minDate: new Date(yyyy - 84, mm2, dd),  // More info about minDate: http://docs.mobiscroll.com/2-17-1/datetime#!opt-minDate
    ...
    $(this).on("click", "#btn_GetPrices_Travel", function (event) {
        ////////debugger;
        var Insured = $("input.TravelInsuredNumber")

        for (i = 1; i <= Insured.length; i++) {
            if ($("#txt_TravelDOB_" + i).is(':visible')) {
                if ($.trim($("#txt_TravelDOB_" + i).val()) == "") {
                    jq_DisplayMessage("MSG_GeneralError", "", "Please Fill All Insured DOB.!");
                    $("#txt_TravelDOB_" + i).focus();
                    return false;
                }
        ...
        //    }
            ...
            for (i = 1; i <= parseInt(_TravelOffer.InsNo() === undefined ? "0" : _TravelOffer.InsNo().toString()) ; i++) {
                //////debugger;
                _TravelOffer.My_Members.unshift(new Member_family(-1, '', jq_parseDate2($("#txt_TravelDOB_" + i).val()), 0, '', '', 0, 0, 0, 0, 0, 0, 0));
            }
    ...
    $("#DDL_Tr_Membres").bind("change", function (event, ui) {
        ...
        if ($(this).val() != '' && $(this).val() !== "undefined" && $(this).val() !== null && _TravelOffer.Travel_INSURED.BIRTHDATE() != "") {
            for (i = 1; i <= 7 ; i++) {
                $("#li_TravelInsured_" + i).hide();
            }
            //Empty invisible texboxes
            var InsuredNum = $("input.TravelInsuredNumber")
            for (i = 1; i <= InsuredNum.length; i++) {
                if (!$("#txt_TravelDOB_" + i).is(':visible')) {
                    $("#txt_TravelDOB_" + i).val("");
                }
            }

            for (i = 1; i <= $(this).val() ; i++) {
                $("#li_TravelInsured_" + i).show("fast");
            }
        }
        else
        {
    ...
    try {
      //////debugger;
        ...
        if (JSON.parse(sessionStorage.getItem("_Selected_Travel_Plan")) != null) {
            ...
            ko.mapping.fromJS(List_Members, MyApp_GVM.List_Members);

            List_CountyOfResidance = [{ "COUNTRIES_ID": "14", "COUNTRIES_NAME": "Lebanon" }];
            MyApp_GVM.List_CountyOfResidance = ko.mapping.fromJS(List_CountyOfResidance);
            _TravelOffer.Travel_INSURED.COUNTRIES_ID(TravelOfferEdited.Travel_INSURED.COUNTRIES_ID());

            if (TravelOfferEdited.My_Members().length > 0) {
                $('#DDL_Tr_Membres').val(TravelOfferEdited.My_Members().length).trigger("change");
            }
            else {
                $('#DDL_Tr_Membres').val(null).trigger("change");
            }

            for (i = 1; i <= TravelOfferEdited.My_Members().length; i++) {
                $("#txt_TravelDOB_" + i).val(moment(TravelOfferEdited.My_Members()[i - 1].MEMBER_BIRTHDATE()).format("DD/MM/YYYY"));
                $("#li_TravelInsured_" + i).show("fast");
            }
